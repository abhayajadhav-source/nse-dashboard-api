...
def scan_patterns():
    ...
    Defensive design: every step wrapped in try/except, no unhandled
    exceptions can propagate. If something goes wrong, the response
    body contains diagnostic info — never a bare 500.

    Query params:
      ?limit=N       — cap universe size (default 20, max 100)
      ?diag=1        — return diagnostics only (no pattern detection)

    Universe priority:
      1. momentum_snapshot table (cron-fed list of "active" stocks)
      2. stock_list.get_all_symbols() — the NIFTY 200 universe
    """
    # Wrap the WHOLE thing in a top-level try/except so a 500 is impossible
    try:
        try:
        ...
        diag_mode = request.args.get("diag") in ("1", "true", "yes")

        # ===== Step 1: figure out the universe =====
        universe = []
        universe_source = "none"

        # Try momentum_snapshot first
        momentum_error = None
        if DATABASE_URL:
            try:
                with _conn() as conn:
                ...
                if universe:
                    universe_source = "momentum_snapshot"
            except Exception as e:
        ...
        if diag_mode or not universe:
            return jsonify({
                ...
                "universe_preview":    universe[:5] if universe else [],
                "database_url_set":    bool(DATABASE_URL),
                "momentum_error":      momentum_error,
                "stock_list_error":    stock_list_error,
                "pattern_detector_importable": _check_pattern_detector_importable(),
                "results":             [],
                "scanned":             0,
            }), 200

        # ===== Step 2: run pattern detection per symbol =====
        results = []
        failures = []
        for sym in universe:
            try:
                ...
                failures.append(f"{sym}: {type(e).__name__}: {str(e)[:100]}")
                logger.warning("Scan: %s failed: %s", sym, e)
                continue

            # _detect_patterns_cached may return an "error" key on failure
            if patterns.get("error"):
                failures.append(f"{sym}: {str(patterns['error'])[:100]}")
                continue

            try:
                candles = patterns.get("candlesticks", []) or []
                charts  = patterns.get("chart_patterns", []) or []
                ...
                results.append({
                    ...
                    "chart_pattern_count": len(charts),
                    "bullish_count":       summary.get("bullish_count", 0),
                    "bearish_count":       summary.get("bearish_count", 0),
                    "formed_count":        summary.get("formed_count", 0),
                    "forming_count":       summary.get("forming_count", 0),
                    "patterns":            (candles + charts)[:3],
                })
            except Exception as e:
                failures.append(f"{sym}: post-detect: {type(e).__name__}: {str(e)[:100]}")
                logger.warning("Scan: %s post-detect: %s", sym, e)

        # ===== Step 3: sort + return =====
        try:
            results.sort(
                key=lambda r: (r["formed_count"],
                                abs(r["bullish_count"] - r["bearish_count"])),
                reverse=True
            )
        except Exception as e:
            logger.warning("Scan: sort failed (continuing): %s", e)

        return jsonify({
            "scanned":         len(universe),
            "with_patterns":   len(results),
            "failure_count":   len(failures),
            "first_failures":  failures[:5],   # show first 5 for debugging
            "universe_source": universe_source,
            "results":         results,
            "fetched_at":      datetime.utcnow().isoformat() + "Z",
        }), 200
        ...
        logger.exception("Scan: top-level exception: %s", e)
        return jsonify({
            "error":         f"Unexpected error: {type(e).__name__}",
            "error_detail":  str(e)[:500],
            "scanned":       0,
            "results":       [],
        }), 200   # Return 200 with error body, not 500


def _check_pattern_detector_importable() -> dict:
    """Diagnostic: check if pattern_detector module is importable + working."""
    try:
        from pattern_detector import detect_all_patterns
        return {"importable": True, "function_present": True}
    except Exception as e:
        return {"importable": False, "error": f"{type(e).__name__}: {str(e)[:200]}"}
